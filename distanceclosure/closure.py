...
def distance_closure(D, kind='metric', minkowski_par=1.0, algorithm='dijkstra', weight='weight', only_backbone=False, verbose=False, self_loops=False, *args, **kwargs):
    ...
    kind : string
        Type of closure to compute: [``metric``, ``ultrametric``, ``product``, ``minkowski``].
    ...
    self_loops : bool
        Compute the closure for self-loops
    ...
    Examples
    --------
    >>> distance_closure(D, kind='metric', algorithm='dijkstra', weight='weight', only_backbone=True, self_loop=False)
            ...
            if (u, v) in edges_seen or u == v:
                ...
                if not G.has_edge(u, v):
                    if not only_backbone:
                        G.add_edge(u, v, **{weight: np.inf, 'trig_distance': length, 'is_trig': False})
                else:
                    G[u][v]['trig_distance'] = length
                    G[u][v]['is_trig'] = True if (length == G[u][v][weight]) else False
        i += 1
